...
const APP_TOKEN_KEY = 'spotmusic_spotify_app_token';
const APP_TOKEN_EXPIRES_KEY = 'spotmusic_spotify_app_token_expires';
  ...
  public async ensureAccessToken(): Promise<boolean> {
    ...
    if (localStorage.getItem(REFRESH_TOKEN_KEY) && this.configured) {
      try {
        ...
        if (spotifyClient.hasAccessToken) return true;
      } catch {
        this.disconnect(false);
      }
    }
